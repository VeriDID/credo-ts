...
import type { AttributeSpec, WorkflowInstanceData } from '..'
import type { GuardEnv } from '../engine/GuardEvaluator'

describe('Engine helpers', () => {
  test('AttributePlanner materialize: context/static/compute', () => {
    const plan: Record<string, AttributeSpec> = {
      a: { source: 'context', path: 'user.name', required: true },
      b: { source: 'static', value: 42 },
      c: { source: 'compute', expr: "join('', ['hi-','there'])" },
    }
    const instance = { context: { user: { name: 'Alice' } } } as unknown as WorkflowInstanceData
  ...
  test('AttributePlanner required throws missing_attributes', () => {
    const plan: Record<string, AttributeSpec> = { req: { source: 'context', path: 'x', required: true } }
    const instance = { context: {} } as unknown as WorkflowInstanceData
    expect(() => AttributePlanner.materialize(plan, instance)).toThrow()
  })

  test('GuardEvaluator evalGuard with JMESPath truthy/falsey', () => {
    const env: GuardEnv = { context: { a: 1, b: 0 }, participants: {}, artifacts: {} }
    expect(GuardEvaluator.evalGuard('context.a', env)).toBe(true)
    expect(GuardEvaluator.evalGuard('context.b', env)).toBe(false)
  })

  test('GuardEvaluator evalValue returns selected JSON piece', () => {
    const env: GuardEnv = { context: { a: { x: 'ok' } }, participants: {}, artifacts: {} }
    expect(GuardEvaluator.evalValue('context.a.x', env)).toBe('ok')
  })

  test('AttributePlanner compute error returns undefined unless required', () => {
    const badPlan: Record<string, AttributeSpec> = {
      x: { source: 'compute', expr: 'invalid++expr' },
      y: { source: 'compute', expr: 'invalid++expr', required: true },
    }
    const inst = { context: {} } as unknown as WorkflowInstanceData
